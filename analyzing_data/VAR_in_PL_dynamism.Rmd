...
- PPDA (aquí buscamos valores cercanos a cero)
...
df_var_details <- get_df(4,is_work_computer) %>% select(-c('big_six','big_six_h','big_six_a'))
```

```{r merge_data,include=FALSE}
#Antes de empezar a unir la data vamos a aplicar funciones agregadas a la tabla df_gw para obtener las métricas por temporada y GW
df_gw <- df_gw %>% group_by(season,team_name,GW) %>% summarise(yellow_cards = sum(yellow_cards),red_cards = sum(red_cards),
                                                               penalties_conceded = sum(penalties_conceded),
                                                               own_goals = sum(own_goals),fouls = sum(fouls), 
                                                               goals_conceded = max(goals_conceded), 
                                                               num_passes = sum(completed_passes),
                                                               cross_passes = sum(open_play_crosses))
#Ahora agrupamos más para tener la data a nivel temporada
df_gw <- df_gw %>% group_by(season,team_name) %>% summarise(matches = max(GW),yellow_cards = sum(yellow_cards),
                                                            red_cards = sum(red_cards), penalties_conceded = sum(penalties_conceded),
                                                            own_goals = sum(own_goals),fouls = sum(fouls), 
                                                            total_goals_conceded = sum(goals_conceded), 
                                                            avg_goals_conceded = round(mean(goals_conceded),2), 
                                                            num_passes = sum(num_passes),cross_passes = sum(cross_passes))
...
#Antes de hacer el cruce, homologamos los nombres de los equipos
...
#Cruzamos la primera data
df_prev <- df_gw %>% inner_join(df_xg,by = c('season','team_name' = 'team1')) %>% rename(avg_xg_conceded = avg_xg, 
                                                                                         total_xg_conceded = total_xg)
#Hacemos el total de tiros hechos por el equipo contrario por temporada y equipo
df_shots <- df_shots %>% group_by(season,HomeTeam) %>% summarise(total_shots_conceded = sum(AS), 
                                                                 total_shots_target_conceded = sum(AST))
...
#Hacemos el segundo cruce de data
df_prev1 <- df_prev %>% inner_join(df_shots,by = c('season','team_name'='HomeTeam'))
```
