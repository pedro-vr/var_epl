...
plt_assists <- ggplot(assists_per_GW,aes(GW,sum_assists)) + geom_line(color="brown") + geom_point(color="orange") + geom_hline(yintercept=mean(assists_per_GW$sum_assists),colour="blue") + labs(x='jornada',y="total asistencias",title = "Total de asistencias por jornada en PL (2016-2022)")
```
...
Al obetner la gráfica final podemos observar un comportamiento bastante peculiar al final de la temporada, en primer lugar, podemos observar que desde la jornada 1 (GW=1) hasta aproximadamente la jornada 38 (GW=38) el número total de asistencias eran mayores a 100 y la gran mayoría por encima de la media (línea horizontal azul), sin embargo, podemos observar a su vez que, a partir de aproximadamente la jornada 39, todos los equipos presentan una reducción muy considerable en su número de asistencias, éste cae incluso por debajo de 50 (muy cercano a 25) y parece mantenerse bajo hasta fin de temporada. Este comportamiento que podemos deducir general en todas las temporadas es bastante interesante y vale la pena analizarlo con detalle.

**Número de asistencias al principio vs fin de temporada**

Como pudimos observar en la gráfica anterior, parece ser que el número total de asistencias reduce considerablemente al final de cada temporada (aprox después de la jornada 38). Esto nos induce a analizar con mayor cuidado este comportamiento. 

En primer lugar, analizaremos este comportamiento jornada tras jornada pero en esta ocasión separando cada temporada para poder analizarlos por separado.
