...
get_df <- function(df_number,is_work_computer=F){
  ...
  df_name <- switch (as.character(df_number),
    '1' = '/premier_league.csv', #listo
    '2' = '/pl_overturns_var.csv', #listo
    '3' = '/var_details.csv',#listo
    '4' = '/var_details1.csv', #listo
    '5' = '/stats_epl.csv', #listo
    '6' = '/fpl_merged_seasons.csv',
    '7' = '/fpl_merged_players.csv',#listo
    '8' = '/fpl_merged_gw.csv' #listo
    )
...
```{r prob}
pl_matches <- get_df(df_number = 2)
pl_matches %>% filter(!is.na(team))
```
