...
#script en donde incluimos las funciones referentes a la tabla players_standard_stats
source("functions_scripts/df_players_standard_stats.R", local = knitr::knit_global())
```
...
#Obtenemos el username de Git
git_username <- my_git_credentials()$username
#Obtenemos el pw de Git
git_pw <- my_git_credentials()$pw
...
files_names <- get_pull_files_names()
#Nombre de los archivos con los que se guardarán en el folder del repo propio
nombres_finales <- get_pull_destination_files_names()
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(players_teams_catalog_1617,players_teams_catalog_1718,players_teams_catalog_1819,players_teams_catalog_1920,players_teams_catalog_2021,players_teams_catalog_2122)
```

```{r update_fpl_data,include=FALSE}
#En esta parte actualizamos la data proveniente del repo de GitHub de la FPL

#Hacemos el pull dependiendo de qué compu sea la que trabaje y si es necesario hacer el pull
pull_fpl_data(ruta_fpl_repo,git_username,git_pw,params$is_work_computer,params$do_pull,files_names,ruta_raw,nombres_finales)
```
...
premier_league$team1 <- sub_value_df(premier_league$team1,'Brighton and Hove Albion', 'Brighton & Hove Albion')
...
premier_league$team2 <- sub_value_df(premier_league$team2,'Brighton and Hove Albion', 'Brighton & Hove Albion')
...
#Homologamos el nombre de los equipos
ovrt_df$team <- sub_value_df(ovrt_df$team,'Newcastle', 'Newcastle United')
ovrt_df$team <- sub_value_df(ovrt_df$team,'Leeds', 'Leeds United')
ovrt_df$team <- sub_value_df(ovrt_df$team,'Norwich', 'Norwich City')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(ovrt_df_19_20,ovrt_df_20_21,ovrt_df_21_22)
```
...
ovrt_var$team <- sub_value_df(ovrt_var$team,'Leeds', 'Leeds United')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(des_df,des_df_19_20,des_df_20_21,des_df_21_22)
```
...
var_details$team1 <- sub_value_df(var_details$team1,'Leeds', 'Leeds United')
var_details$team1 <- sub_value_df(var_details$team1,'Newcastle', 'Newcastle United')
var_details$team2 <- sub_value_df(var_details$team2,'Bournemouth', 'AFC Bournemouth')
var_details$team2 <- sub_value_df(var_details$team2,'Brighton', 'Brighton & Hove Albion')
var_details$team2 <- sub_value_df(var_details$team2,'Leeds', 'Leeds United')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(var_details_19_20,var_details_20_21,var_details_21_22)
```
...
var_details1$team1 <- sub_value_df(var_details1$team1,'Leeds', 'Leeds United')
var_details1$team1 <- sub_value_df(var_details1$team1,'Newcastle', 'Newcastle United')
var_details1$team2 <- sub_value_df(var_details1$team2,'Bournemouth', 'AFC Bournemouth')
var_details1$team2 <- sub_value_df(var_details1$team2,'Brighton', 'Brighton & Hove Albion')
var_details1$team2 <- sub_value_df(var_details1$team2,'Leeds', 'Leeds United')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(var_details1_19_20,var_details1_20_21,var_details1_21_22)
```
...
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Cardiff', 'Cardiff City')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Huddersfield', 'Huddersfield Town')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Hull', 'Hull City')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Leeds', 'Leeds United')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Newcastle', 'Newcastle United')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Stoke', 'Stoke City')
stats_epl$HomeTeam <- sub_value_df(stats_epl$HomeTeam,'Swansea', 'Swansea City')
...
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Cardiff', 'Cardiff City')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Huddersfield', 'Huddersfield Town')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Hull', 'Hull City')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Leeds', 'Leeds United')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Newcastle', 'Newcastle United')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Stoke', 'Stoke City')
stats_epl$AwayTeam <- sub_value_df(stats_epl$AwayTeam,'Swansea', 'Swansea City')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(stats_1617,stats_1718,stats_1819,stats_1920,stats_2021,stats_2122)
```
...
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Bournemouth','AFC Bournemouth')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Cardiff', 'Cardiff City')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Huddersfield', 'Huddersfield Town')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Hull', 'Hull City')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Leeds', 'Leeds United')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Newcastle', 'Newcastle United')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Stoke', 'Stoke City')
cleaned_merged_seasons$team_name <- sub_value_df(cleaned_merged_seasons$team_name,'Swansea', 'Swansea City')
...
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Cardiff', 'Cardiff City')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Huddersfield', 'Huddersfield Town')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Hull', 'Hull City')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Leeds', 'Leeds United')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Newcastle', 'Newcastle United')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Stoke', 'Stoke City')
cleaned_merged_seasons$opp_team_name <- sub_value_df(cleaned_merged_seasons$opp_team_name,'Swansea', 'Swansea City')
...
#Quitamos los valores NA de la columna team_name
merged_cleaned_players$team_name[is.na(merged_cleaned_players$team_name)] <- "" 

#Homologamos el nombre de los equipos
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Swansea', 'Swansea City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Bournemouth','AFC Bournemouth')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Brighton','Brighton & Hove Albion')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Cardiff', 'Cardiff City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Huddersfield', 'Huddersfield Town')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Hull', 'Hull City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Leeds', 'Leeds United')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Leicester','Leicester City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Man City','Manchester City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Man Utd','Manchester United')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Newcastle', 'Newcastle United')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Norwich','Norwich City')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Sheffield Utd','Sheffield United')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Spurs','Tottenham Hotspur')
merged_cleaned_players$team_name <- sub_value_df(merged_cleaned_players$team_name,'Stoke', 'Stoke City')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(cleaned_players_1617,cleaned_players_1718,cleaned_players_1819,cleaned_players_1920,cleaned_players_2021,cleaned_players_2122)
```
...
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Bournemouth','AFC Bournemouth')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Cardiff', 'Cardiff City')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Huddersfield', 'Huddersfield Town')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Hull', 'Hull City')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Leeds', 'Leeds United')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Newcastle', 'Newcastle United')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Stoke', 'Stoke City')
merged_gw_formatted$team_name <- sub_value_df(merged_gw_formatted$team_name,'Swansea', 'Swansea City')
...
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Cardiff', 'Cardiff City')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Huddersfield', 'Huddersfield Town')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Hull', 'Hull City')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Leeds', 'Leeds United')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Newcastle', 'Newcastle United')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Stoke', 'Stoke City')
merged_gw_formatted$opponent_team <- sub_value_df(merged_gw_formatted$opponent_team,'Swansea', 'Swansea City')
...
#Eliminamos los dfs auxiliares para quedarnos solo con el último
rm(merged_gw,merged_gw_1617,merged_gw_1718,merged_gw_1819,merged_gw_1920,merged_gw_2021,merged_gw_2122)
```
...
poss_stats$team <- sub_value_df(poss_stats$team,'Newcastle Utd','Newcastle United')
...
poss_stats$team <- sub_value_df(poss_stats$team,'Huddersfield', 'Huddersfield Town')
...
La tabla número 11 que se presenta aquí contiene información acerca del porcentaje (%) de tiempo de acción que tuvo cada equipo en general durante toda la temporada dentro de cada tercio de la cancha en cada partido de la PL. La data que se presenta aquí es desde la temporada 2016/2017 al día de hoy. 

Algunas de las variables que se presentan en esta tabla son las siguientes:

1. team_name: Nombre del equipo de la PL

2. own_third: Porcentaje (%) de tiempo que ocupó el equipo correspondiente dentro del primer tercio de la cancha (propia cancha) durante toda la temporada correspondiente

3. opposition_third: Porcentaje (%) de tiempo que ocupó el equipo correspondiente dentro del último tercio de la cancha (cancha rival) durante toda la temporada correspondiente

Los datos aquí consultados se obtuvieron de la página [WhoScored.com](https://www.whoscored.com/Regions/252/Tournaments/2/Seasons/6335/Stages/13796/TeamStatistics/England-Premier-League-2016-2017) (página de ejemplo para la temporada 2016-2017).
...
#hacemos cambio de nombre de los equipos para homologar con demas dfs
action_zones$team_name <- sub_value_df(action_zones$team_name,'Bournemouth','AFC Bournemouth')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Brighton','Brighton & Hove Albion')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Cardiff','Cardiff City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Huddersfield', 'Huddersfield Town')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Hull','Hull City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Leeds','Leeds United')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Leicester','Leicester City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Newcastle','Newcastle United')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Norwich','Norwich City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Stoke','Stoke City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Swansea','Swansea City')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Tottenham','Tottenham Hotspur')
action_zones$team_name <- sub_value_df(action_zones$team_name,'West Bromwich','West Brom')
action_zones$team_name <- sub_value_df(action_zones$team_name,'Wolverhampton','Wolves')
...
## Tabla 12: "pl_players_standard_stats". Fuente: FBREF

La tabla número 12 que se presenta aquí contiene información acerca de algunas de las estadísticas más importantes que se necesitan para medir el performance de los jugadores dentro de la PL.

Algunas de las variables que se presentan en esta tabla son las siguientes:

1. team_name: Nombre del equipo de la PL

2. goals_scored: Número total de goles anotados por un jugador para un equipo en específico dentro de determinada temporada de la PL

3. xg: Número total de goles esperados efectuados por el jugador en cuestión dentro de una temporada en específico de la PL

Los datos aquí consultados se obtuvieron de la página [FBREF](https://fbref.com/en/comps/9/2021-2022/stats/2021-2022-Premier-League-Stats) (página de ejemplo para la temporada 2021-2022).

Para información más detallada de las variables que se presentan en esta tabla se puede consultar el repositorio de [GitHub](https://github.com/pedro-vr/var_epl/tree/main/tables/dictionaries).

```{r players_standard_stats,include=FALSE}
#leemos la data
df_ply_std_stats <- get_players_standard_stats(ruta_raw)

#Leemos la data correspondiente a GK
df_ply_gk <- get_players_gk_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas de tiro del jugador
df_ply_shoots <- get_players_shooting_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas de pases del jugador
df_ply_passes <- get_players_passing_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas del tipo de pases del jugador
df_ply_passes_types <- get_players_passing_types_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas de creaciones de tiros y goles del jugador
df_ply_goal_shot_creation <- get_players_goals_shoots_creation_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas de acciones defensivas del jugador
df_ply_defensive_actions <- get_players_defensive_actions_stats(ruta_raw)

#Leemos la data correspondiente a las estadísticas de posesión del balón del jugador
df_ply_possession <- get_players_possession_stats(ruta_raw)

#hacemos cambio de nombre de los equipos en df de standard stats para homologar con demas dfs
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Bournemouth','AFC Bournemouth')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Brighton','Brighton & Hove Albion')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Huddersfield','Huddersfield Town')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Manchester Utd','Manchester United')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Newcastle Utd','Newcastle United')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Sheffield Utd','Sheffield United')
df_ply_std_stats$team_name <- sub_value_df(df_ply_std_stats$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de GK para homologar con demas dfs
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Bournemouth','AFC Bournemouth')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Brighton','Brighton & Hove Albion')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Huddersfield','Huddersfield Town')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Manchester Utd','Manchester United')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Newcastle Utd','Newcastle United')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Sheffield Utd','Sheffield United')
df_ply_gk$team_name <- sub_value_df(df_ply_gk$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de shoots para homologar con demas dfs
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Bournemouth','AFC Bournemouth')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Brighton','Brighton & Hove Albion')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Huddersfield','Huddersfield Town')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Manchester Utd','Manchester United')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Newcastle Utd','Newcastle United')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Sheffield Utd','Sheffield United')
df_ply_shoots$team_name <- sub_value_df(df_ply_shoots$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de passes para homologar con demas dfs
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Bournemouth','AFC Bournemouth')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Brighton','Brighton & Hove Albion')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Huddersfield','Huddersfield Town')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Manchester Utd','Manchester United')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Newcastle Utd','Newcastle United')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Sheffield Utd','Sheffield United')
df_ply_passes$team_name <- sub_value_df(df_ply_passes$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de passes_types para homologar con demas dfs
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Bournemouth','AFC Bournemouth')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Brighton','Brighton & Hove Albion')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Huddersfield','Huddersfield Town')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Manchester Utd','Manchester United')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Newcastle Utd','Newcastle United')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Sheffield Utd','Sheffield United')
df_ply_passes_types$team_name <- sub_value_df(df_ply_passes_types$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de possession para homologar con demas dfs
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Bournemouth','AFC Bournemouth')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Brighton','Brighton & Hove Albion')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Huddersfield','Huddersfield Town')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Manchester Utd','Manchester United')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Newcastle Utd','Newcastle United')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Sheffield Utd','Sheffield United')
df_ply_possession$team_name <- sub_value_df(df_ply_possession$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de goal_shot_creation para homologar con demas dfs
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Bournemouth','AFC Bournemouth')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Brighton','Brighton & Hove Albion')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Huddersfield','Huddersfield Town')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Manchester Utd','Manchester United')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Newcastle Utd','Newcastle United')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Sheffield Utd','Sheffield United')
df_ply_goal_shot_creation$team_name <- sub_value_df(df_ply_goal_shot_creation$team_name,'Tottenham','Tottenham Hotspur')

#hacemos cambio de nombre de los equipos en df de defensive_actions para homologar con demas dfs
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Bournemouth','AFC Bournemouth')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Brighton','Brighton & Hove Albion')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Huddersfield','Huddersfield Town')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Manchester Utd','Manchester United')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Newcastle Utd','Newcastle United')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Sheffield Utd','Sheffield United')
df_ply_defensive_actions$team_name <- sub_value_df(df_ply_defensive_actions$team_name,'Tottenham','Tottenham Hotspur')

#Modificamos la columna nation para que quede con el formato deseado en el df de standard stats
df_ply_std_stats$nation <- sapply(df_ply_std_stats$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de standard stats
df_ply_std_stats$pos <- sapply(df_ply_std_stats$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de goalkeeping
df_ply_gk$nation <- sapply(df_ply_gk$nation,nationality_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de shoots
df_ply_shoots$nation <- sapply(df_ply_shoots$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de shoots
df_ply_shoots$pos <- sapply(df_ply_shoots$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de passes
df_ply_passes$nation <- sapply(df_ply_passes$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de passes
df_ply_passes$pos <- sapply(df_ply_passes$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de passes_types
df_ply_passes_types$nation <- sapply(df_ply_passes_types$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de passes_types
df_ply_passes_types$pos <- sapply(df_ply_passes_types$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de goal_shot_creation
df_ply_goal_shot_creation$nation <- sapply(df_ply_goal_shot_creation$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de goal_shot_creation
df_ply_goal_shot_creation$pos <- sapply(df_ply_goal_shot_creation$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de defensive_actions
df_ply_defensive_actions$nation <- sapply(df_ply_defensive_actions$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de defensive_actions
df_ply_defensive_actions$pos <- sapply(df_ply_defensive_actions$pos,position_formatted)
#Modificamos la columna nation para que quede con el formato deseado en el df de possession
df_ply_possession$nation <- sapply(df_ply_possession$nation,nationality_formatted)
#Modificamos la columna pos para que quede con el formato deseado en el df de possession
df_ply_possession$pos <- sapply(df_ply_possession$pos,position_formatted)

#De la tabla para GK nos quedamos solo con las columnas que nos interesan
df_ply_gk <- df_ply_gk %>% select(-c('nation','pos','age','born','match_ply','starts','min','mp_90','gk_wins','gk_deuces','gk_losses'))
#De la tabla para shoots nos quedamos solo con las columnas que nos interesan
df_ply_shoots <- df_ply_shoots %>% select(-c('nation','pos','age','born','mp_90','npxg','sh_pen_kicks','sh_pen_kicks_att','xg','sh_goals_scored'))
#De la tabla para pases nos quedamos solo con las columnas que nos interesan
df_ply_passes <- df_ply_passes %>% select(-c('nation','pos','age','born','mp_90','assists','prog_passes','xag'))
#De la tabla para pases_types nos quedamos solo con las columnas que nos interesan
df_ply_passes_types <- df_ply_passes_types %>% select(-c('nation','pos','age','born','mp_90','live_ball_passes','dead_ball_passes','free_kicks_passes','throw_ins','inswinging_corner_kicks','outswinging_corner_kicks','straight_corner_kicks','passes_completed','passes_attempted'))
#De la tabla para goal_shot_creation nos quedamos solo con las columnas que nos interesan
df_ply_goal_shot_creation<- df_ply_goal_shot_creation %>% select(-c('nation','pos','age','born','mp_90','sca_live_passes','sca_dead_passes','sca_fouls_drawn','gca_live_passes','gca_dead_passes','gca_fouls_drawn'))
#De la tabla para defensive_actions nos quedamos solo con las columnas que nos interesan
df_ply_defensive_actions <- df_ply_defensive_actions %>% select(-c('nation','pos','age','born','mp_90','dribbles_challenged'))
#De la tabla para possession nos quedamos solo con las columnas que nos interesan
df_ply_possession <- df_ply_possession %>% select(-c('nation','pos','age','born','mp_90','prog_carries','prog_passes_received','live_touches'))

#Unimos las dos tablas para tener la información completa
df_ply_stats <- df_ply_std_stats %>% left_join(df_ply_gk,by = c('player','team_name','season')) %>% left_join(df_ply_shoots,by = c('player','team_name','season')) %>% left_join(df_ply_passes,by = c('player','team_name','season')) %>% left_join(df_ply_passes_types,by = c('player','team_name','season')) %>% left_join(df_ply_goal_shot_creation,by = c('player','team_name','season')) %>% left_join(df_ply_defensive_actions,by = c('player','team_name','season')) %>% left_join(df_ply_possession,by = c('player','team_name','season'))

#Eliminamos los dfs individuales y solo nos quedamos con el general
rm(df_ply_gk,df_ply_std_stats,df_ply_shoots,df_ply_passes,df_ply_passes_types,df_ply_goal_shot_creation,df_ply_defensive_actions,df_ply_possession)

#Escribimos la tabla final a la carpeta local

#declaramos la ruta final de la tabla
ruta_final_ply_std_stats <- paste0(ruta_guardado,'pl_players_general_stats.csv')
#Lo escribimos
write_csv(df_ply_stats,ruta_final_ply_std_stats)
```

Una vista general de algunos registros de esta tabla es la siguiente:

```{r players_standard_stats_view}
head(df_ply_stats)
```
